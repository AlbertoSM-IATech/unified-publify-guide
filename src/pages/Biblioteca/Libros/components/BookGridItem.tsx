...
export const BookGridItem = ({
  ...
  return <div className="card-hover group rounded-lg border bg-card shadow-sm">
      <div className="flex h-40 overflow-hidden">
        ...
        <div className="relative h-full w-28 bg-muted flex-shrink-0">
          {libro.imageUrl ? <div className="h-full w-full overflow-hidden">
              <AspectRatio ratio={16 / 25.6} className="h-full">
                <img src={libro.imageUrl} alt={libro.titulo} className="h-full w-full object-cover" />
              </AspectRatio>
            </div> : <div className="flex h-full w-full items-center justify-center text-muted-foreground/50">
              <BookOpen size={40} />
            </div>}
        </div>
        ...
        <div className="flex flex-1 flex-col justify-between p-5 py-[7px]">
          <div className="space-y-2.5">
            ...
            {libro.subtitulo && <p className="line-clamp-1 text-sm text-muted-foreground">
                {libro.subtitulo}
              </p>}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${getContentColor(libro.contenido)}`}>
                {libro.contenido}
              </span>
            </div>
          </div>
