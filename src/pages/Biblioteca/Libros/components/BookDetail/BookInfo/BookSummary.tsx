  ...
  return <motion.div initial={{
    ...
    duration: 0.3
      ...
      <div className="flex items-center gap-2">
        ...
        <Button variant="link" className="text-lg p-0 h-auto" onClick={() => book.authorPageUrl && handleOpenUrl(book.authorPageUrl)}>
          {book.autor}
        </Button>
      </div>
      ...
      <div className="grid gap-3 py-0 my-[20px]">
        ...
        {isEditing ? <Select defaultValue={book.estado} onValueChange={handleStatusChange}>
          ...
          </Select> : <Badge variant="outline" className={cn("capitalize", book.estado === "Publicado" ? "bg-green-100 text-green-800" : "", book.estado === "Borrador" ? "bg-yellow-100 text-yellow-800" : "", book.estado === "En revisión" ? "bg-blue-100 text-blue-800" : "", book.estado === "Archivado" ? "bg-gray-100 text-gray-800" : "")}>
            {book.estado}
          </Badge>}
      </div>
      ...
      <div className="grid gap-3">
        ...
        {isEditing ? <Select defaultValue={book.contenido} onValueChange={handleContentTypeChange}>
            <SelectTrigger className="hover:border-[#FB923C] transition-colors duration-200">
          ...
          </Select> : <div className="text-sm">{book.contenido}</div>}
      </div>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          {book.fechaPublicacion && <div className="flex items-center text-sm text-muted-foreground">
              <Calendar className="w-4 h-4 mr-2" />
              <span>Publicación: {formatDate(book.fechaPublicacion)}</span>
            </div>}
          {book.fechaLanzamiento && <div className="flex items-center text-sm text-muted-foreground">
              <Calendar className="w-4 h-4 mr-2" />
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {getAmazonLink() && <Button variant="outline" size="sm" className="w-full justify-start" onClick={() => handleOpenUrl(getAmazonLink() || '')}>
              <ExternalLink className="w-4 h-4 mr-2" />
              Ver en Amazon ({selectedFormat})
            </Button>}
          {book.landingPageUrl && <Button variant="outline" size="sm" className="w-full justify-start" onClick={() => handleOpenUrl(book.landingPageUrl || '')}>
