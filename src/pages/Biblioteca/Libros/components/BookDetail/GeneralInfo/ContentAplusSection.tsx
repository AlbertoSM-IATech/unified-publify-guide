...
export const ContentAplusSection = ({
  book,
  isEditing,
  form
}: ContentAplusSectionProps) => {
  ...
  const handleOpenPreview = (fileToPreview: File | ContentAPlusFile) => {
    ...
    if (fileToPreview instanceof File) {
      ...
      // Existing file (ContentAPlusFile object)
  ...
  return <div className="grid gap-6 mt-3 p-4 border rounded-md bg-muted/30">
      ...
      <Tabs defaultValue="text" className="w-full">
        ...
        <TabsContent value="text">
          <ContentAPlusTextSection isEditing={isEditing} form={form} initialTextContent={book.contenidoAPlus} />
        </TabsContent>
        
        <TabsContent value="gallery">
          <ContentAPlusGallerySection isEditing={isEditing} form={form} initialFiles={book.contenidoAPlusFiles || []} onOpenPreview={handleOpenPreview} />
        </TabsContent>
      </Tabs>

      <ContentAPlusPreviewDialog isOpen={isPreviewOpen} onClose={handleClosePreview} fileName={previewFileName} fileUrl={previewUrl} fileType={previewFileType} />
    </div>;
};