    ...
    <div className="space-y-6 mt-8">
      ...
      <Card className="p-5 space-y-4 shadow-sm border-slate-200 dark:border-slate-700">
        ...
        <div className="grid gap-3">
          <Label htmlFor="targetAge">Rango de edad</Label>
          {isEditing ? (
            <FormField
        ...
        <div className="grid gap-3">
          <Label htmlFor="targetGender">Género</Label>
          {isEditing ? (
            <FormField
              control={form.control}
              name="targetGender"
            ...
            <div className="border rounded-md p-2 bg-card shadow-sm">
              {book.targetGender || "No definido"}
            </div>
          )}
        </div>
        
        <div className="grid gap-3">
          <Label htmlFor="targetInterests">Intereses</Label>
          {isEditing ? (
            <FormField
              control={form.control}
            ...
            <div className="border rounded-md p-2 bg-card shadow-sm min-h-[50px]">
              {book.targetInterests || "No definido"}
            </div>
          )}
        </div>
      </Card>
      ...
      <Card className="p-5 space-y-4 shadow-sm border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-2 text-blue-500 mb-1">
          <Target size={18} />
          <h4 className="text-md font-semibold">Posicionamiento</h4>
        </div>
        
        <div className="grid gap-3">
          <Label htmlFor="marketPosition">Posición en el mercado</Label>
          {isEditing ? (
            <FormField
              control={form.control}
              name="marketPosition"
            ...
            <div className="border rounded-md p-2 bg-card shadow-sm min-h-[50px]">
              {book.marketPosition || "No definido"}
            </div>
          )}
        </div>
        
        <div className="grid gap-3">
          <Label htmlFor="competitorBooks">Libros competidores</Label>
          {isEditing ? (
            <FormField
              control={form.control}
      ...
      <Card className="p-5 space-y-4 shadow-sm border-slate-200 dark:border-slate-700">
        ...
        <div className="grid gap-3">
          ...
          {isEditing ? (
            ...
            <div className="border rounded-md p-2 bg-card shadow-sm min-h-[75px]">
