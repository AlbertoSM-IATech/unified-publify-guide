  ...
  return <div className="flex min-h-screen flex-col items-center justify-center bg-background p-4">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          <h1 className="font-heading text-3xl font-bold text-orange-400">
            {isVerified ? "Email verificado" : "Verifica tu email"}
          </h1>
          <p className="mt-2 text-muted-foreground">
            {isVerified ? "Tu correo ha sido verificado correctamente" : token ? "Estamos verificando tu correo electrónico..." : "Hemos enviado un correo de verificación a tu dirección email"}
          </p>
        </div>
        ...
        {!token && !isVerified && <div className="space-y-4">
            <div className="rounded-lg border p-4 bg-muted/20">
              <p className="text-sm">
                {email ? <>Hemos enviado un correo a <span className="font-semibold">{email}</span>.</> : <>Hemos enviado un correo a tu dirección de email.</>}
                {' '}Por favor, haz clic en el enlace de verificación para completar tu registro.
              </p>
            </div>
            
            <Button onClick={handleResendEmail} variant="outline" className="w-full" disabled={resendingEmail}>
        ...
        {isVerified && <div className="space-y-4 text-center">
