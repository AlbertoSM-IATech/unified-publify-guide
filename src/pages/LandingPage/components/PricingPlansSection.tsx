...
const PlanCard = ({ plan, index, openDialog }: PlanCardProps) => {
  const Icon = plan.icon;
  const isPlus = plan.id === "plus";
  const { ref, style } = useTilt<HTMLDivElement>({ max: plan.highlighted ? 5 : 3, scale: 1.01 });
    ...
    >
      {plan.highlighted && (
        <>
          <div
            aria-hidden
            className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[110%] aspect-square border border-primary/15 rounded-full animate-[spin_40s_linear_infinite]"
          />
          <div
            aria-hidden
            className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[125%] aspect-square border border-accent/10 rounded-full animate-[spin_60s_linear_infinite_reverse]"
          />
          <div
            aria-hidden
            className="pointer-events-none absolute inset-0 -z-10 blur-3xl opacity-70 rounded-2xl"
            style={{ background: "radial-gradient(circle at 50% 20%, hsl(var(--primary) / 0.18), transparent 65%)" }}
          />
        </>
      )}
      ...
      <div
        ref={ref}
        style={style}
        className={`relative flex flex-col p-8 rounded-2xl border bg-card transition-colors ${
          plan.highlighted
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${plan.highlighted ? "bg-primary/15" : "bg-accent/10"}`}>
            <Icon className={`w-5 h-5 ${plan.highlighted ? "text-primary" : "text-accent"}`} />
          </div>
          ...
          {isPlus && plan.earlyPrice ? (
            <>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Desde junio: <span className="line-through">{plan.standardPrice}€/mes</span>
              </p>
            </>
          ) : (
            <>
              <div className="flex items-baseline gap-2">
                <span className="text-5xl font-bold text-foreground">{plan.standardPrice}€</span>
                <span className="text-sm text-muted-foreground">/mes</span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">Precio estándar desde junio</p>
            </>
          )}
        </div>
        ...
        <ul className="space-y-3 mb-8 flex-1">
          {plan.features.map((feature, idx) => (
            <li key={idx} className="flex items-start gap-3 text-sm text-foreground">
              <Check className={`w-4 h-4 flex-shrink-0 mt-0.5 ${plan.highlighted ? "text-primary" : "text-accent"}`} />
              <span>{feature}</span>
            </li>
          ))}
        </ul>
        ...
        {plan.disabled && (
          <p className="text-xs text-muted-foreground text-center mt-3">Disponible más adelante</p>
        )}
      </div>
    </motion.div>
  );
